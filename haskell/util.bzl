...
def _source_prefix(source: Artifact, module_name: str) -> str:
    ...
    # If the module name matches exactly, no prefix
    if module_name == module_name_for_file:
        return ""

    # If the module name is a suffix of the file's module name, extract the prefix
    if module_name_for_file.endswith("." + module_name):
